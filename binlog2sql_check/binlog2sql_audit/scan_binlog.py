...
from scan_binlog_util import command_line_args, update_dml_count, create_unique_file, is_dml_event, event_type
...
class Binlog2sql(object):
    ...
    def __init__(self, connection_settings, start_file=None, start_pos=None, only_schemas=None, only_tables=None,
        ...
        self.no_pk, self.stop_never, self.back_interval = (
            no_pk, stop_never, back_interval)
        ...
        with self.connection as cursor:
            ...
            if self.start_file not in bin_index:
                raise ValueError(
                    'parameter error: start_file %s not in mysql server' % self.start_file)

            def binlog2i(x): return x.split('.')[1]
            ...
            if not self.server_id:
                raise ValueError('missing server_id in %s:%s' % (
    ...
    def process_binlog(self):
        ...
        count_dict = {"insert": 0, "delete": 0, "update": 0}
        # to simplify code, we do not use flock for tmp_file.
        tmp_file = create_unique_file('%s.%s' % (
            self.conn_setting['host'], self.conn_setting['port']))
        with open(tmp_file, "w") as f_tmp, self.connection as cursor:
            for binlog_event in stream:
                if isinstance(binlog_event, QueryEvent) and binlog_event.query == 'BEGIN':
                    e_start_pos = last_pos
                    trans_start_time = binlog_event.timestamp
                    count_dict = {"insert": 0, "delete": 0, "update": 0}

                if isinstance(binlog_event, QueryEvent) and not self.only_dml:
                    count_dict = update_dml_count(cursor=cursor, binlog_event=binlog_event,
                                                              flashback=False, no_pk=self.no_pk, count_dict=count_dict)
                elif is_dml_event(binlog_event) and event_type(binlog_event) in self.sql_type:
                    for row in binlog_event.rows:
                        count_dict = update_dml_count(cursor=cursor, binlog_event=binlog_event, no_pk=self.no_pk,
                                                                  row=row, flashback=False, e_start_pos=e_start_pos, count_dict=count_dict)

                elif isinstance(binlog_event, XidEvent):
                    last_pos = binlog_event.packet.log_pos
                    trans_size = last_pos - e_start_pos
                    trans_end_time = binlog_event.timestamp
                    duration = trans_end_time - trans_start_time
                    print("trans_size: %s insert_count:%s update_count%s delete_count%s duration:%s" % (trans_size, count_dict["insert"], count_dict["update"], count_dict["delete"], duration))

                if isinstance(binlog_event, RotateEvent):
                    last_pos = 4
                elif not (isinstance(binlog_event, RotateEvent) or isinstance(binlog_event, FormatDescriptionEvent)):
                    last_pos = binlog_event.packet.log_pos
...
if __name__ == '__main__':
    ...
    conn_setting = {'host': args.host, 'port': args.port,
                    'user': args.user, 'passwd': args.password, 'charset': 'utf8'}
